...
uses
  ...
  {$ENDIF}
    ...
    ActnList, ExtCtrls, ComCtrls, Grids, Spin, SortGrid, StrUtils, KGrids,
    KControls, KDialogs, KGraphics, KFunctions, New.Grids, Types;
...
procedure TForm2.StringGrid1PickListSelect(Sender: TObject);
begin
  //ShowMessage('PickListSelect');
end;

procedure TForm2.StringGrid1SelectEditor(Sender: TObject; aCol, aRow: Integer;
  var Editor: TWinControl);
begin
  Case ACol Of
    5: TComboBox( Editor ).Style:= csDropDownList;
...
begin
 Try
  ...
  ShowMessage( StringGrid1.Cells[2, 2] );
...
Var
 ...
 Begin
  ...
  //   TMyTextCell(KGrid1.Cell[2, LRowIndex]).AsBoolean:= False;
...
begin
  ...
  //    KGrid1.CellPainter.CheckBoxHPadding:= Trunc( KGrid1.ColWidths[ACol] / 2 ) - 5
  //
  //  {$IFDEF LCLQT}
...
Initialization
 { *** Altera o Valor de Conversão Boolean -> String Padrão *** }
 TrueBoolStrs:=  ['True'];
 FalseBoolStrs:= ['False'];
